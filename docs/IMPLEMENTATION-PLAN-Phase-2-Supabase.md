...
- **Roles:** tabla `profiles` (1:1 con `auth.users`) con columna `role` (`'admin' | 'user'`). El **Viewer** público es la ausencia de sesión; su acceso se resuelve con políticas RLS de lectura abierta sobre `series`.
  > **Posterior a F7:** el rol `user` se retiró del proyecto. Queda un único rol `admin` (migración `20260801120000_single_admin_role.sql`, que ajusta CHECK, default y el trigger `handle_new_user`), y con él desaparecieron la pantalla `/users` y la ruta `/login`. El resto de esta sección describe el estado de la Fase 2 y se conserva como histórico.
