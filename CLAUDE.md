...
- **Three roles** — Viewer (no login, read-only, default; resolved with public-read RLS on `series`), User (logged in, but read-only: no create/edit/delete), Admin (CRUD all series + user management). **Writes are admin-only**: `canCreateSeries`/`canEditSeries`/`canDeleteSeries` in `src/utils/permissions.ts` all require `role === 'admin'`, and `/series/new` + `/series/:id/edit` are guarded with `roles={['admin']}`. This is an app-level restriction — Supabase RLS still lets an authenticated `user` write their own series via the API. Route protection via `<ProtectedRoute>` guard; session and roles handled by **Supabase Auth** + a `profiles` table.
